...
`nova-submit` ships as two files on the [releases page](https://github.com/jcarbonnell/nova/releases): the compiled `.wasm` and its capabilities sidecar. Download both into the same directory, then install.

On your IronClaw agent:
...
ironclaw tool install /home/agent/nova-submit.wasm
```

Confirm it installed with its permissions:

```bash
ironclaw tool list        # nova-submit should show caps: ✓
ironclaw tool info nova-submit
```

> The capabilities file **must** sit next to the `.wasm` and share its filename stem (`nova-submit.capabilities.json` next to `nova-submit.wasm`). Without it, the tool installs with no permissions (`caps: ✗`) and cannot make any network calls.
...
nova-submit-tool/
  ...
  nova-submit-tool.capabilities.json  capabilities manifest (released as nova-submit.capabilities.json)
```
