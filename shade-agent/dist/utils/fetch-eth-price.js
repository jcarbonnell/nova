...
exports.getEthereumPriceUSD = getEthereumPriceUSD;
// Fetch ETH price from OKX
async function getETHPriceFromOKX() {
    try {
        const response = await fetch("https://www.okx.com/api/v5/market/ticker?instId=ETH-USDT");
        if (!response.ok) {
...
async function getEthereumPriceUSD() {
    try {
        ...
        if (okxPrice === null)
            return Math.round(coinbasePrice * 100);
        if (coinbasePrice === null)
            return Math.round(okxPrice * 100);
        // Calculate average, multiply by 100 and round to integer
        const averagePrice = Math.round(((okxPrice + coinbasePrice) / 2) * 100);
        console.log(`Average ETH Price: $${(averagePrice / 100).toFixed(2)} (OKX: $${okxPrice}, Coinbase: $${coinbasePrice})`);
        return averagePrice;
    }
    catch (error) {
...
//# sourceMappingURL=fetch-eth-price.js.map