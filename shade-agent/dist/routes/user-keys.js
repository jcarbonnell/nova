...
import { storeUserKey, retrieveUserKey, checkAccount, generateApiKey, hasApiKey, verifyApiKey, } from '../lib/services/user-keys.js';
// ────────────────────────────────────────────────
...
userKeys.post('/retrieve', validate(RetrieveSchema), async (c) => c.json(await retrieveUserKey(body(c, RetrieveSchema))));
userKeys.post('/check', validate(CheckSchema), async (c) => c.json(await checkAccount(body(c, CheckSchema))));
userKeys.post('/generate-api-key', validate(ApiKeyLookupSchema), async (c) => c.json(await generateApiKey(body(c, ApiKeyLookupSchema))));
userKeys.post('/has-api-key', validate(ApiKeyLookupSchema), async (c) => c.json(await hasApiKey(body(c, ApiKeyLookupSchema))));
