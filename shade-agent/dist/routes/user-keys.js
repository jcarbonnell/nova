...
// Database setup
const userKeysDb = new better_sqlite3_1.default('./nova-user-keys.db');
userKeysDb.exec(`
...
const userKeys = new hono_1.Hono();
// ========== STORE ENDPOINT ==========
...
userKeys.get('/', async (c) => {
    try {
        const info = await (0, shade_agent_js_1.agentInfo)();
        const count = userKeysDb.prepare('SELECT COUNT(*) as count FROM user_account_keys').get();
        return c.json({
            status: 'healthy',
