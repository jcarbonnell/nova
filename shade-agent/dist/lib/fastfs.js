...
export function deriveFastfsPathPrefix(groupId) {
...
export function newRelativePath(groupId) {
...
// with backend=FastFS.
export function encodeFastfsLocation(rel, predecessor = FASTFS_PREDECESSOR, receiver = FASTFS_RECEIVER) {
    return `${predecessor}/${receiver}/${rel}`;
}
export function parseFastfsLocation(location) {
    const i = location.indexOf('/');
    const j = location.indexOf('/', i + 1);
    if (i < 0 || j < 0)
        throw new ApiError(400, 'BAD_LOCATION', 'Malformed FastFS location');
    return {
...
// ────────────────────────────────────────────────
export async function retrieve(loc) {
    if (loc.backend === 'ipfs')
        return retrieveIpfsLegacy(loc.location);
...
export function assertFastfsEnvelopeFinalized(outcome, receiver = FASTFS_RECEIVER) {
    ...
    if (!outcome?.transaction?.hash) {
        throw new ApiError(500, 'FASTFS_NOT_FINALIZED', 'FastFS envelope did not finalize (no transaction hash in outcome)');
    }
    const status = outcome?.status;
    const failure = status?.Failure ?? status?.failure;
    if (!failure)
        return; // executed as success — unexpected, but data is on-chain; fine
    const kind = JSON.stringify(failure);
    if (kind.includes('CodeDoesNotExist') && kind.includes(receiver))
        return; // the expected no-op
    throw new ApiError(500, 'FASTFS_UNEXPECTED_STATUS', `FastFS envelope finalized with an unexpected execution status ` +
...
const FASTFS_GAS = 300000000000000n; // 300 TGas (spike-validated; the failed no-op burns far less)
...
export function borshFastfsUpload(rel, mime, content) {
...
export function borshFastfsDelete(rel) {
    return Buffer.concat([
...
export async function uploadAt(ciphertext, relativePath) {
    ...
    const outcome = await signAndBroadcastFunctionCall(FASTFS_RECEIVER, FASTFS_METHOD, args, FASTFS_GAS, 0n, { tolerateFailure: true });
...
export async function upload(ciphertext, groupId) {
    return uploadAt(ciphertext, newRelativePath(groupId));
}
export async function remove(loc) {
