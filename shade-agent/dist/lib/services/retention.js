...
async function writeRegistry(groups) {
 ...
 */
export async function registerRetentionGroup(input) {
 ...
 * NOT block the clear.
 */
export async function deregisterRetentionGroup(input) {
    const { group_id } = input;
    const groups = await readRegistry();
    if (!groups.includes(group_id)) {
 ...
 */
export async function scanRetention(input) {
    const groups = await readRegistry();
    ...
    for (const group_id of groups) {
        try {
            const retention_days = await viewOrThrow(rpcUrl, contractId, 'get_group_retention', { group_id });
            ...
            const expired = await viewOrThrow(rpcUrl, contractId, 'get_expired_transactions', { group_id });
...
const NOVA_GAS = 100000000000000n; // 100 TGas — matches MCP's call_contract default
...
function decodeSuccessValue(outcome) {
...
async function fetchExpiredDetailed(contractId, groupId) {
    const args = Buffer.from(JSON.stringify({ group_id: groupId }));
    const outcome = await signAndBroadcastFunctionCall(contractId, 'get_expired_transactions_detailed', args, NOVA_GAS, 0n);
...
export async function executeRetention(input) {
    ...
    for (const [trans_id, location] of expired) {
        ...
        try {
            ...
            results.push({ trans_id, location, destroyed: false, error: `bad location: ${err.message}` });
        ...
        try {
            ...
            results.push({ trans_id, location, destroyed: false, error: `shred failed: ${err.message}` });
            log('warn', 'retention_shred_failed', { group_id, trans_id: trans_id.slice(0, 12), error: err.message });
            continue;
        }
        ...
        try {
            ...
            bookkeepingError = `fastfs remove failed: ${err.message}`;
        }
        ...
        if (!bookkeepingError) {
            try {
                ...
                bookkeepingError = `on-chain tombstone failed: ${err.message}`;
            }
        }
        if (bookkeepingError) {
