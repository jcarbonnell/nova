...
let enclaveSigner: { publicKey: Uint8Array; privateKey: Uint8Array } | null = null;
 ...
 */
export async function initializeEnclaveSigner(): Promise<void> {
  if (enclaveSigner) {
...
export function deriveKey(salt: string, length: number = 32): Uint8Array {
  ...
  return hkdf(
    ...
    new TextEncoder().encode('nova-v1'),
    length
  );
}
 ...
 */
export function decryptBlob(enc: string): Uint8Array {
