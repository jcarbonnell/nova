 ...
 */
let WALLET_VERIFIER_NEAR: Near | null = null;

function getWalletVerifierNear(): Near {
  if (WALLET_VERIFIER_NEAR) return WALLET_VERIFIER_NEAR;
 ...
 */
export function issueWalletNonce(): string {
  return walletNonceStore.issueNonce();
}

/** Result of a wallet SIWN verification. `code` mirrors better-near-auth's two
 *  error codes for §5.11-B wire-alignment; the route maps it onto ApiError. */
export type WalletSigninResult =
  | { ok: true; account_id: string; public_key: string }
  | { ok: false; code: 'UNAUTHORIZED_NONCE_REPLAY' | 'UNAUTHORIZED'; reason?: string };
...
export async function verifyWalletSignin(
  ...
  try {
    ...
    let sigValid = false;
    try {
      sigValid = await verifyNep413Signature(
        signedMessage,
        {
          message,
