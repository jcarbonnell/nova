...
function fileKeyIdFor(groupId: string, fileRef: string): string {
  return sha256Hex(`file-key:${groupId}:${fileRef}`);
}

export interface FileKeyInput {
  group_id: string;
  file_ref: string;            // per-upload-unique (the FastFS relativePath)
  token?: string;
  account_id?: string;
  contract_id?: string;
}
...
async function authorizeForGroup(
  groupId: string, contractId: string, network: string,
  ...
  const authorized = await viewFunction(
    getRpcUrl(network), contractId, 'is_authorized', { group_id: groupId, user_id },
  );
  if (!authorized) throw new ApiError(403, 'UNAUTHORIZED', 'Unauthorized');
}
...
export async function generateFileKey(input: FileKeyInput) {
...
export async function getFileKey(input: FileKeyInput) {
  const { group_id, file_ref, token, account_id, contract_id } = input;
  const { contractId, network } = resolveContract(contract_id, group_id);
  await authorizeForGroup(group_id, contractId, network, token, account_id);

  const blob = await getBlobFromKV(fileKeyIdFor(group_id, file_ref));
  if (!blob) throw new ApiError(404, 'FILE_KEY_NOT_FOUND', 'No key for this file');
  ...
  if (decrypted.equals(TOMBSTONE)) throw new ApiError(404, 'FILE_DELETED', 'File deleted');

  const { v, w } = JSON.parse(decrypted.toString('utf8')) as { v: string | null; w: string };
