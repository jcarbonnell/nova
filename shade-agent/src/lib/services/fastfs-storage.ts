...
type PrepareFileUploadInput = z.infer<typeof PrepareFileUploadSchema>;
type FinalizeFileUploadInput = z.infer<typeof FinalizeFileUploadSchema>;
type RetrieveFileInput = z.infer<typeof RetrieveFileSchema>;

// The FileFormatV1 record the SDK produces (Step 3). Shade stores it verbatim and
// hands it back on retrieve — it does not interpret it (the SDK's decodeFile does).
// Loosely typed on purpose: the SDK owns the schema; Shade is a passthrough store.
type FileFormat = Record<string, unknown>;

const fileMetaIdFor = (groupId: string, fileRef: string): string =>
  sha256Hex(`file-meta:${groupId}:${fileRef}`);
...
export async function finalizeFileUpload(input: FinalizeFileUploadInput) {
  ...
  await storeBlobToKV(
    fileMetaIdFor(group_id, file_ref),
    encryptBlob(Buffer.from(metaRecord, 'utf8')),
  );
...
// path.
export async function retrieveFile(input: RetrieveFileInput) {
  ...
  if (metaBlob) {
    ...
    format = (parsed?.format ?? null) as FileFormat | null;
  }
