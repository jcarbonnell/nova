...
type ApiKeyLookupInput = z.infer<typeof ApiKeyLookupSchema>;
type VerifyApiKeyInput = z.infer<typeof VerifyApiKeySchema>;
...
export async function storeUserKey(input: StoreInput) {
...
export async function retrieveUserKey(input: RetrieveInput) {
...
export async function checkAccount(input: CheckInput) {
 ...
 */
async function readApiKeyRecord(accountId: string): Promise<{ v: number; hash: string } | null> {
 ...
 */
function deriveApiKeyValue(accountId: string, version: number): string {
...
async function writeApiKeyRecord(accountId: string, v: number, hash: string): Promise<void> {
  const hashKeyId = sha256Hex(`api-hash:${accountId}`);
...
async function resolveApiKeyTarget(input: ApiKeyLookupInput): Promise<string> {
  ...
  if (account_id || wallet_id) {
    log('warn', 'api_key_unauth_branch_rejected', {
      account_hash: hashForLog((account_id || wallet_id) as string),
    });
    throw new ApiError(501, WALLET_DISABLED.code, WALLET_DISABLED.message);
  }
...
export async function generateApiKey(input: ApiKeyLookupInput) {
...
export async function rotateApiKey(input: ApiKeyLookupInput) {
  const targetAccountId = await resolveApiKeyTarget(input);
...
export async function hasApiKey(input: ApiKeyLookupInput) {
  const targetAccountId = await resolveApiKeyTarget(input);
 ...
 * VERIFY API KEY.
 *
 * Returns a DOMAIN OUTCOME, not an HTTP response — the adapter maps it.
 * This endpoint's wire contract is genuinely status-dependent and NOT uniform:
 *   - bad format      → 401 { valid: false, error: 'Invalid format' }
 *   - no key stored   → 401 { valid: false, error: 'No API key configured' }
 *   - hash MISMATCH   → 200 { valid: false, account_id, network }   ← 200, not 401
 * The 200-with-valid:false case is the subtle one; preserve it exactly.
 *
 * (The frontend's session-token Path 0 reads `errorData.error` on !ok and
 * `verifyData.valid` on ok — so the `valid` field in the 401 bodies is in fact
 * unused. Normalising these to plain ApiErrors is a safe, deliberate follow-up;
 * it is NOT done here, because this refactor changes no bytes on the wire.)
 */
export type VerifyApiKeyOutcome =
  | { kind: 'invalid_format' }
  | { kind: 'no_key_configured' }
  | { kind: 'checked'; valid: boolean; account_id: string; network: string };

export async function verifyApiKey(input: VerifyApiKeyInput): Promise<VerifyApiKeyOutcome> {
