...
/** Outcome of a nonce check. A single reason field keeps the route's error
 *  mapping trivial: any non-'ok' maps to UNAUTHORIZED_NONCE_REPLAY. */
export type NonceCheck =
  | { ok: true }
  | { ok: false; reason: 'unknown' | 'consumed' | 'expired' };

interface NonceRecord {
  issuedAt: number; // ms, per the injected clock
  consumed: boolean;
}

export interface WalletNonceStoreOptions {
  ttlMs?: number;
  /** Injectable clock (ms). Defaults to Date.now. Harness overrides this. */
  now?: () => number;
}
...
export class WalletNonceStore {
  ...
  constructor(opts: WalletNonceStoreOptions = {}) {
   ...
   */
  checkNonce(nonceHex: string): NonceCheck {
   ...
   */
  consumeNonce(nonceHex: string): boolean {
    const rec = this.store.get(nonceHex);
    if (!rec) return false;
